...
import nskafka.errors as Errors
from nskafka.future import Future
from nskafka.protocol.api import RequestHeader
from nskafka.protocol.commit import GroupCoordinatorResponse
from nskafka.protocol.types import Int32
...
class BrokerConnection(object):
    ...
    def __init__(self, host, port, afi, **configs):
        ...
        # proxy代理转换
        self.host, self.port = self.getproxyinfo(self.host, self.port)
    ...
    # proxy代理信息转换
    def getproxyinfo(self, host, port):
      hostinfo = host + ':' + str(port)
      proxy_info = self.config['report_proxy_map'].get(hostinfo, '')
      if proxy_info != '':
        proxy_array = proxy_info.split(':')
        return proxy_array[0], int(proxy_array[1])
      else:
        return host, port
