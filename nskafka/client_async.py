...
class KafkaClient(object):
    ...
    def __init__(self, **configs):
        ...
        # network proxy info.
        self._report_host_map = {}
        if self.config['report_proxy_info'] != '':
            proxylist = self.config['report_proxy_info'].split(',')
            for item in proxylist:
                proxyinfo = item.split('@')
                self._report_host_map[proxyinfo[0]] = proxyinfo[1]
        self.config['report_proxy_map'] = self._report_host_map 
